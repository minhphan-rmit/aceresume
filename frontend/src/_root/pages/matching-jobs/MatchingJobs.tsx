...
function MatchedJobs() {
  ...
  const fetchAllCVs = async () => {
    try {
      const response = await fetch(` ${process.env.REACT_APP_API_BASE_URL}/resume/${userId}/get_all_resume`);
      if (!response.ok) {
  ...
  const handleSearch = async () => {
    try {
      setLoading(true);
      const response = await fetch(` ${process.env.REACT_APP_API_BASE_URL}/job/${encodeURIComponent(searchQuery)}/find-jobs?user_id=${userId}&resume_id=${temporaryResumeId}&location=${selectedLocation}`, {
        method: 'POST',
        headers: {
