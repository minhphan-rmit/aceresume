...
function MatchedJobs() {
  ...
  const handleSearch = async () => {
    try {
      setLoading(true);
      const response = await fetch(` https://ace-resume-backend-saahlmc6ha-as.a.run.app/api/aceresume/job/${encodeURIComponent(searchQuery)}/find-jobs?user_id=${userId}&resume_id=${temporaryResumeId}&location=${selectedLocation}`, {
        method: 'POST',
        headers: {
