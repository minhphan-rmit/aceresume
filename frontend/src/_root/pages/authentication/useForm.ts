...
const useForm = ({ initialValues }: UseFormProps) => {
  ...
  const register = async (e: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const response = await axios.post(' https://ace-resume-backend-saahlmc6ha-as.a.run.app/api/aceresume/register', submissionData, {
        headers: {
  ...
  const signin= async (e: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      console.log('Submission data:', submissionData);
      const response = await axios.post(' https://ace-resume-backend-saahlmc6ha-as.a.run.app/api/aceresume/login', submissionData, {
        headers: {
          'Content-Type': 'application/x-www-form-urlencoded;charset=UTF-8'
        },
      });
