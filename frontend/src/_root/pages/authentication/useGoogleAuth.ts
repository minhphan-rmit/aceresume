...
const useGoogleAuth = () => {
  ...
  const sendEmailToBackend = async (email: string) => {
    try {
      ...
      const response = await axios.post(` ${process.env.REACT_APP_API_BASE_URL}/google/login`, { email },{
  ...
  const sendRegisteredDetailsToBackend = async (email: string, user_name: string, password:string) => {
    try {
     ...
     console.log(submissionData);
      const response = await axios.post(` ${process.env.REACT_APP_API_BASE_URL}/google/register`, submissionData,{
